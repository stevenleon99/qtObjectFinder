...
void TestManipulator::followFingers(const QVector2D& lastP1,
                                    ...
                                    QVariant varCamera)
{
    ...
    QSizeF zoomCurrent =
        dirLast.length() / dirCurrent.length() * camera->viewSize();

    const double shiftAngle = GmVectorUtils::angleRad(dirLast, dirCurrent);

    QVector3D viewDir = (camera->center() - camera->eye()).normalized();

    QVector2D mouseCenterLast =
        QVector2D(parentItem->width(), parentItem->height()) / 2.0 - lastP1;

    QVector3D screenXlast =
        QVector3D::normal(viewDir, camera->upVector().normalized()) *
        mouseCenterLast.x() * camera->viewSize().width() / parentItem->width();
    QVector3D screenYlast = camera->upVector().normalized() *
                            mouseCenterLast.y() * camera->viewSize().height() /
                            parentItem->height();

    QVector3D fP1a = camera->center() - screenXlast + screenYlast;

    QVector2D mouseCenterCurrent =
        QVector2D(parentItem->width(), parentItem->height()) / 2.0 - currentP1;
    ...
    QVector3D screenXcurrent =
        QVector3D::normal(viewDir, camera->upVector().normalized()) *
        mouseCenterCurrent.x() * camera->viewSize().width() /
        parentItem->width();
    QVector3D screenYcurrent =
        camera->upVector().normalized() * mouseCenterCurrent.y() *
        camera->viewSize().height() / parentItem->height();
...
void TestManipulator::rotation(const QVector2D& lastPos,
                               const QVector2D& pos,
                               QVariant varCamera)
{
    QQuickItem* parentItem = qobject_cast<QQuickItem*>(parent());
