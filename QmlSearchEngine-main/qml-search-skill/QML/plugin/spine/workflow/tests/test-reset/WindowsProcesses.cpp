...
bool WindowsProcesses::launchAppIntoDifferentSession(const QString& program,
                                                     ...
                                                     bool consoleUser)
{
        ...
        if (hProcess)
        {
            ...
            if (!::OpenProcessToken(hProcess,
                                    TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY |
                                        ...
                                        TOKEN_ADJUST_SESSIONID | TOKEN_READ |
                                        TOKEN_WRITE,
                                    &hPToken))
            {
            ...
            SetTokenInformation(hUserTokenDup, TokenSessionId,
    ...
    bool success = CreateProcessAsUser(
        ...
        workingDir.isEmpty()
            ? 0
            : (wchar_t*)workingDir.utf16(),  // name of current directory
...
void WindowsProcesses::printLastError(const QString& msg)
{
    ...
    // Display the message
    qDebug() << "WARNING: " << msg.toStdString().c_str()
             << "[ failed with error: " << eNum << "("
             << QString::fromWCharArray(sysMsg).toStdString().c_str() << ")]";
}
