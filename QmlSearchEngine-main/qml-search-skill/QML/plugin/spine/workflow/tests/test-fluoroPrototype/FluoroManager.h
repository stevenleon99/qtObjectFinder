...
{
    Q_OBJECT
    Q_PROPERTY(QVector2D bodyPosteriorLatImg READ bodyPosteriorLatImg WRITE
                   setBodyPosteriorLatImg NOTIFY pointsChanged)
    Q_PROPERTY(QVector2D bodyAnteriorLatImg READ bodyAnteriorLatImg WRITE
                   setBodyAnteriorLatImg NOTIFY pointsChanged)

    Q_PROPERTY(QVector2D pedicleAinside READ pedicleAinside WRITE
                   setPedicleAinside NOTIFY pointsChanged)
    Q_PROPERTY(QVector2D pedicleBinside READ pedicleBinside WRITE
                   setPedicleBinside NOTIFY pointsChanged)

    Q_PROPERTY(float pedicleAwidth READ pedicleAwidth WRITE setPedicleAwidth
                   NOTIFY pointsChanged)
    Q_PROPERTY(float pedicleBwidth READ pedicleBwidth WRITE setPedicleBwidth
                   NOTIFY pointsChanged)
    ...
    Q_PROPERTY(bool screwAactive READ screwAactive WRITE setScrewAactive NOTIFY
                   pointsChanged)
...
signals:
    void pointsChanged();
    void cursorPosition(const QString& uuid,
                        const QVector3D& ptA,
                        const QVector3D& ptB);
...
public slots:
    ...
    void mousePressed(const QString& uuid,
                      const QVector2D& pt,
                      float imageScale);
    ...
    QVector3D closestPointOnLineAfromLineB(QVector3D lineAp0,
                                           QVector3D lineAp1,
                                           QVector3D lineBp0,
                                           QVector3D lineBp1);


    QObject* image(const QString& uuid)
    {
        return m_fluoroImages.contains(uuid) ? m_fluoroImages[uuid] : 0;
    }
