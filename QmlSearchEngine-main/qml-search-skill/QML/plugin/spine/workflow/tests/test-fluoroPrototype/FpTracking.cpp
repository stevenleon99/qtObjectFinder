...
FpTracking::FpTracking(QObject* parent)
    : QObject(parent)
{
    m_socket = new QTcpSocket(this);
    QObject::connect(m_socket, &QTcpSocket::stateChanged, this,
                     &FpTracking::tcpStateChanged);

    m_client = new QJsonRpcSocket(m_socket, this);
    QObject::connect(m_client, &QJsonRpcSocket::messageReceived, this,
                     &FpTracking::messageReceived);
...
void FpTracking::connect(const QString& server)
{
    if (m_socket->state() == QAbstractSocket::UnconnectedState)
    {
        ...
        {
            qDebug() << "Trouble connecting to gps-controller: "
...
void FpTracking::messageReceived(const QJsonRpcMessage& message)
{
    ...
    for (const QString& key : keys)
    {
        ...
        Tracking::Tracker* trkr =
            Tracking::TrackerManagerSingleton::Instance()->tracker(key);

        if (trkr)
        {
            QString xfmKey = trkr->camera_X_rom_uuid();
            TransformationObject* camera_X_rom =
                TransformationManagerSingleton::Instance()->transform(xfmKey);
            bool displayed =
                trkElement.elementStatus() == TrackingEnums::Visible ||
                trkElement.elementStatus() == TrackingEnums::PartialView;
            trkr->setAccuracyStatus(trkElement.accCheckStatus());

            if (!displayed && camera_X_rom)
            {
                TransformationManagerSingleton::Instance()
                    ->removeReferencedXforms(xfmKey);
            }
            else if (trkElement.elementStatus() == TrackingEnums::Disabled &&
                     camera_X_rom)
            {
                TransformationManagerSingleton::Instance()
                    ->removeReferencedXforms(xfmKey);
            ...
            else if (displayed)
            {
                if (!camera_X_rom)
                {
                    camera_X_rom = TransformationManagerSingleton::Instance()
                                       ->TransformationManager::transform(
                                           TransformationObject::Camera,
                                           frame.localizerUuid(),
                                           TransformationObject::ROM, key);
                    camera_X_rom->setType(TransformationObject::Live);
                    trkr->setCamera_X_rom_uuid(camera_X_rom->uuid());
                    TransformationManagerSingleton::Instance()
                        ->resetReferencedXforms();
                }
            }
            if (camera_X_rom)
            {
