...
QMap<QString, QMatrix4x4> EventModel::activeTrackers(const QByteArray &json)
{
    ...
    for (QString key :v.keys())
    {
        ...
        if(elMap["status"].toInt() == 0) {
            ...
            QVector3D t = QVector3D(st[0].toFloat(),st[1].toFloat(),st[2].toFloat());
            st = elMap["cameraRelement"].toList();
            QQuaternion q = QQuaternion(st[3].toFloat(),st[0].toFloat(),st[1].toFloat(),st[2].toFloat());
...
{
    QSqlDatabase db = QSqlDatabase::database(EpicDbHelper::connectionName());
    QString qStr("SELECT x,y,z FROM instrumentlandmarks WHERE instrument_uuid = '%1' AND landmark_type = 'Tip_ROM';");
    ...
    if(q.first())
    {
        return QVector3D(q.value(0).toFloat(),q.value(1).toFloat(),q.value(2).toFloat());
    }
...
void EventModel::fluoroAccuracy()
{
    ...
    QString probeUuid = "66b9ce17-717f-4ffe-83e0-0b7649631812"; // Accuracy Probe

    QString probe2Uuid = "e59606ad-9fc9-4afa-93dd-179933447eb4"; // Pedical Probe
...
//    QString drbUuid = "171defab-d434-43ce-abb6-6a5caf3852db";
    ...
    Tracking::TrackerManager *trkManager = Tracking::TrackerManagerSingleton::Instance();
    bool abortRun = false;
    ...
    for (const EventData &ed : m_events)
    {
        if(ed.m_event == "AppStart" && ed.m_stringData == "GPS-Client")
        {
            abortRun = false;
            qDebug() << "**********************************************************************************************************************";
            qDebug() << "APP Started" << ed.m_datetime.addSecs(7*60*60).toString();
            qDebug() << "**********************************************************************************************************************";
            if(fm) {
                fm->deleteLater();
                currentPage.clear();
...
//            fm->setActiveStudy(studyKey(ed.m_scanUuid));
            ...
            fluoroTracker_X_DrbMap[ed.m_scanUuid] = ed.m_cameraXtool[fluoroTrackerUuid].inverted() * ed.m_cameraXtool[drbUuid];
        }
        else if(ed.m_event == "CollectPointFluoro" && currentPage != "FluoroCapture" && !abortRun)
        {
            QString activeProbe;
            ...
            if(ed.m_cameraXtool.contains(drbUuid) && !activeProbe.isEmpty())
            {
                ...
                QMatrix4x4 mat = ed.m_cameraXtool[drbUuid].inverted() * ed.m_cameraXtool[activeProbe];
...
//                rtn = updateTransformWithPickedPnts(fm->activeStudy(),sUuid,fluoroTrackerUuid,drbUuid,fluoroTracker_X_DrbMap);
//                rtn = updateTransformWithAlgorithm(fm->activeStudy(),sUuid,fluoroTrackerUuid,drbUuid,fluoroTracker_X_DrbMap);
                ...
                {
//                    qDebug() << "Run Aborted:" << fm->activeStudy();
                    abortRun = true;
                }
...
void EventModel::updateFluoroTransforms(const QString scanUuid,const QString drbUuid,const QString fluoroTrackerUuid,
                                        const QMatrix4x4 &image_X_Fixture,const QMatrix4x4 &fluoroTracker_X_drb,
                                        const QMatrix4x4 &fixture_X_Image_1,const QMatrix4x4 &fixture_X_Image_2)
{
    TransformationManagerSingleton::Instance()->removeReferencedXforms(scanUuid);

    //  Image_X_FluoroTracker
    TransformationObject* t1 = new cranial::managers::TransformationObject();
    t1->setMetaData(TransformationObject::Computed,TransformationObject::Image,scanUuid,TransformationObject::ROM,fluoroTrackerUuid);
    t1->setMatrix(image_X_Fixture);
    t1->setInvertible(false);
    TransformationManagerSingleton::Instance()->registerXform(t1);

    //  Image_X_DRB
    TransformationObject* t2 = new cranial::managers::TransformationObject();
    t2->setMetaData(TransformationObject::Computed,TransformationObject::Image,scanUuid,TransformationObject::ROM,drbUuid);
    ...
    TransformationObject* t3 = new cranial::managers::TransformationObject();
    t3->setMetaData(TransformationObject::Computed,TransformationObject::ROM,drbUuid,TransformationObject::Image,scanUuid);
    ...
    //  DRB_X_Image (2)
    TransformationObject* t4 = new cranial::managers::TransformationObject();
    t4->setMetaData(TransformationObject::Computed,TransformationObject::ROM,drbUuid,TransformationObject::Image,scanUuid);
...
bool EventModel::updateTransformWithPickedPnts(const QString studyUuid,const QString scanUuid[2],const QString &fluoroTrackerUuid,const QString &drbUuid,
const QMap<QString,QMatrix4x4> &fluoroTracker_X_DrbMap)
{
    ...
    Tracking::FluoroTracker *fluoroTracker = qobject_cast<Tracking::FluoroTracker *>(trkManager->tracker(fluoroTrackerUuid));
    fluoroRegistration.setFixture(fluoroTracker->orientation_markers_rom(),fluoroTracker->registration_markers_rom(),fluoroTracker->grid_markers_rom());
    ...
    {
        fluoroRegistration.setImage(EpicDbHelper::getLatestImageData("CapturedImage",scanUuid[idx]));

        QString qS("SELECT * FROM pickpoints WHERE scan_uuid='%1' AND pick_status='good'");
        ...
        while(q.next())
        {
            QVector2D pickedPnt = QVector2D(q.value("x").toFloat(),q.value("y").toFloat());
...
bool EventModel::updateTransformWithAlgorithm(const QString studyUuid,const QString scanUuid[2],const QString &fluoroTrackerUuid,const QString &drbUuid,
const QMap<QString,QMatrix4x4> &fluoroTracker_X_DrbMap)
{
    ...
    for(int idx=0;idx<2;idx++)
    {
        fluoroRegistration.displayDebug(QString("C:/temp/event_") + studyUuid + (idx ? "_LAT" : "_AP") );
        fluoroRegistration.setImage(EpicDbHelper::getLatestImageData("CapturedImage",scanUuid[idx]));
        bool imageFlipped = false;
        if(fluoroRegistration.autoDetect(imageFlipped) && fluoroRegistration.calculateRegistration())
        {
            objectPointsMultiAngles.push_back(fluoroRegistration.finalUsedObjectPoints());
            imagePointsMultiAngles.push_back(fluoroRegistration.finalUsedImagePoints());
        }
        else
        {
    ...
    multiAnlgeFluoroRegMatrices=fluoroRegistration.multiAngleCameraCalibration(objectPointsMultiAngles,imagePointsMultiAngles);
