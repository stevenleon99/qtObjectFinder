...
MainWindow::MainWindow(QWidget *parent) :
    ...
    ui(new Ui::MainWindow)
 {
    ...
    m_processNames << "gps-controller.exe" << "gps-fluoro-capture.exe" << "dataiomanager.exe";

    QSettings qSettings(ConfigSettings::configFileLocation(), QSettings::IniFormat);
...
bool  MainWindow::checkForService()
{
    ...
    {
        QMessageBox::warning(NULL, "Cannot continue", "Watchdog Service Must Be Alive");
        return false;
    }
    ...
    if (checkForService())
    {
        ...
        for (int i = 0; i < originalPids.size(); i++)
        {
            ...
            displayMsg = QString("<b>%1</b> original pid %2 new %3\n")
                .arg(m_processNames.at(i)).arg(originalPids.at(i)).arg(newPids.at(i));
            ui->statusDisplay->appendHtml(displayMsg);

            if ((originalPids.at(i) == newPids.at(i)) ||
                (newPids.at(i) == -1))
            {
                QString msg = QString("Process %1 failed to restart").arg(originalPids.at(i));
