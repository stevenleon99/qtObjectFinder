    ...
    , m_usbStatus(USB_DISCONNECTED)
{
    ...
    connect(db.driver(),SIGNAL(notification(QString,QSqlDriver::NotificationSource,QVariant)),
    ...
    QObject::connect(m_jsonRpcService, &GmQJsonRpcService::messageReceived , this, &PatientManager::dataIoMessageReceived);
    QObject::connect(m_jsonRpcService, &GmQJsonRpcService::isConnectedChanged,this,&PatientManager::connectStateChanged);


//    QJsonRpcMessage reply = m_client->invokeRemoteMethodBlocking("DataManagement.newPatients");
...
void PatientManager::notification(const QString & name, QSqlDriver::NotificationSource ns, const QVariant & payload)
{
    if("new_scan" == name)
    {
...
void PatientManager::dataIoMessageReceived(const QJsonRpcMessage &message)
{
    ...
    if(message.method() == "DicomImport.itemToLoadCount") {
        ...
        QMetaEnum metaEnum = PatientManager::staticMetaObject.enumerator(PatientManager::staticMetaObject.indexOfEnumerator("UsbStatus" ));
        Q_ASSERT(metaEnum.isValid());
        setUsbStatus( static_cast<PatientManager::UsbStatus>(metaEnum.keyToValue(vMap["status"].toString().toLatin1())) );
    }
...
void PatientManager::addPatientToDb(const QString &uuid)
{
    ...
    QMetaObject::invokeMethod(m_jsonRpcService,
                               ...
                               Qt::BlockingQueuedConnection,
                               Q_RETURN_ARG(QJsonRpcMessage, jsonMsg),
                               Q_ARG(QString, "DataManagement.addPatient"),
                               Q_ARG(QVariant, uuid));
...
QString PatientManager::activePatientName() const
{
    if(!m_activePatientKey.isEmpty())
    {
        ...
        if(mIdx.isValid())
        {
            QString firstName = m_patientModel->data(mIdx,ScanCollectionItem::FirstNameRole).toString();
            QString lastName = m_patientModel->data(mIdx,ScanCollectionItem::LastNameRole).toString();
...
void PatientManager::setActiveScanKey(QString activeScanKey)
{
    if (m_activeScanKey == activeScanKey)
    {
        ...
        if(!activeScanKey.isEmpty())
        {
            ...
            QString scanQ("SELECT patient_uuid from Scan where scan_uuid = '%1'");
        ...
        // Temp db Fix....
        QString examQ("SELECT path, file_name from DataFile where file_uuid = '%1'");
        examQ = examQ.arg(fileUuid);
        QSqlQuery q0(examQ,db);
        if(q0.size() != 1)
            return;
        q0.next();
        QString dFile = q0.value(0).toString().trimmed() + QDir::separator() + q0.value(1).toString().trimmed();

        Q_UNUSED(dFile)
//        {
//            EsElapsedTime("Loading DICOM");
//            if (dFile.isEmpty() || !FoviaClientSingleton::Instance()->OpenDICOMFolderOnServer(dFile))
//            {
