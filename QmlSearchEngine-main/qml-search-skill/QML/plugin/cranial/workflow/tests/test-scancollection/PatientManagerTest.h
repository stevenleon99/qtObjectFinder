...
class PatientManager : public QObject
{
    Q_OBJECT
    Q_ENUMS(UsbStatus)
    Q_PROPERTY(QVariantList newPatientList READ newPatientList WRITE setNewPatientList NOTIFY newPatientListChanged)
    Q_PROPERTY(int itemToLoadCount READ itemToLoadCount WRITE setItemToLoadCount NOTIFY itemToLoadCountChanged)
    Q_PROPERTY(UsbStatus usbStatus READ usbStatus WRITE setUsbStatus NOTIFY usbStatusChanged)
    Q_PROPERTY(QObject *patientModel READ patientModel NOTIFY patientModelChanged)

    Q_PROPERTY(QString activeScanKey READ activeScanKey WRITE setActiveScanKey NOTIFY activeScanKeyChanged)
    Q_PROPERTY(QString activePatientKey READ activePatientKey   NOTIFY activeScanKeyChanged)
    Q_PROPERTY(QString activePatientName READ activePatientName NOTIFY activeScanKeyChanged)
...
public:
    ...
    QVariantList newPatientList() const
    {
        return m_newPatientList;
    }

    int itemToLoadCount() const
    {
        return m_itemToLoadCount;
    }

    UsbStatus usbStatus() const
    {
        return m_usbStatus;
    }

    ScanCollectionModel * patientModel() const
    {
