...
FluoroManager::FluoroManager(QObject *parent)
    ...
    ,m_screwAactive(true)
{
    connect(this,&FluoroManager::pointsChanged,this,&FluoroManager::computeRegistration);
}
...
QImage FluoroManager::requestImage(const QString & id, QSize * /*size*/, const QSize & /*requestedSize*/)
{
...
void FluoroManager::mousePressed(const QString &uuid, const QVector2D &pt, float imageScale)
{
    Q_UNUSED(uuid)
    Q_UNUSED(pt)
    Q_UNUSED(imageScale)
...
void FluoroManager::mousePositionChanged(const QString &uuid, const QVector2D &pt)
{
...
QVector3D FluoroManager::closestPointOnLineAfromLineB( QVector3D lineAp0, QVector3D  lineAp1,QVector3D lineBp0, QVector3D  lineBp1)
{
    GmLine lA;
...
{
    if(m_fluoroImages.contains("APImage") && m_fluoroImages.contains("LatImage"))
    {
        GmLine pAi;
        GmLine pBi;
...
void FluoroManager::computeRegistration()
{
    ...
    float apMMPerPix = (pWb-pWa).length() / (m_pedicleBinside_apImg - m_pedicleAinside_apImg).length();
    ...
    qDebug() << m_pedicleAinside_apImg << m_pedicleAwidth_apImg << m_pedicleBinside_apImg << m_pedicleBwidth_apImg;
