...
/*    m_settingsFile = setF;
    ...
    for(int i=0;i<sz;i++)
    {
        ...
        m_ringControlPnts_img[i] = QVector2D(st[0].toFloat(),st[1].toFloat());
    }
    qSettings.endArray();
    ...
    for(int i=0;i<sz;i++)
    {
        qSettings.setArrayIndex(i);
        QStringList st = qSettings.value("point").toStringList();
        mFidLandmarks_fig += QVector3D(st[0].toFloat(),st[1].toFloat(),st[2].toFloat());
    }
    qSettings.endArray();

    QStringList img_X_fixture = qSettings.value("img_X_fixture").toStringList();
    QStringList fixture1_X_img = qSettings.value("fixture1_X_img").toStringList();
    QStringList fixture2_X_img = qSettings.value("fixture2_X_img").toStringList();
    if(img_X_fixture.size() == 16 && fixture1_X_img.size() == 16 && fixture1_X_img.size() == 16 )
    {
        for(int i=0;i<16;i++)
        {
    ...
    m_fixtureUuid = qSettings.value("fluoro_fixture","Unset fixture").toString();
    m_drbUuid = qSettings.value("active_drb","Unset drb").toString();

    //
    QStringList fixRom_X_drbRom_str = qSettings.value("fixRom_X_drbRom").toStringList();
    if(fixRom_X_drbRom_str.size() == 16)
    {
        TransformationObject *fixRom_X_drbRom = TransformationManagerSingleton::Instance()->transform(
    ...
    {
        QStringList quat = landmarkSettings.value("rom_R_fixture").toStringList();
        QStringList trans = landmarkSettings.value("rom_T_fixture").toStringList();
        ...
        for(int i=0;i<sz;i++)
        {
            ...
            QVector3D pnt = QVector3D(st[0].toFloat(),st[1].toFloat(),st[2].toFloat());
...
{
    TransformationObject *img_X_fixture = TransformationManagerSingleton::Instance()->transform(TransformationObject::Image,m_uuid,
              TransformationObject::CAD,m_fixtureUuid);
...
void FluoroImage::captureFixtureXdrb()
{
    ...
    {
        TransformationManagerSingleton::Instance()->removeReferencedXforms(fixRom_X_drbRom->uuid());
