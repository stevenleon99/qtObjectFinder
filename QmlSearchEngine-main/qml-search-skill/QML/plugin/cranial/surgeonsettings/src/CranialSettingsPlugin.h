...
class CranialSettingsPlugin final : public QObject, public drive::itf::plugin::IPlugin
{
    Q_OBJECT
...
public:
    explicit CranialSettingsPlugin(QObject* parent = nullptr);
    ~CranialSettingsPlugin() override = default;

    std::optional<std::string> getWatchdogFile(sys::config::SystemType systemType) const override;
    std::shared_ptr<drive::itf::plugin::IWorkflowPluginDataModel> getWorkflowDataModel()
        const override;
